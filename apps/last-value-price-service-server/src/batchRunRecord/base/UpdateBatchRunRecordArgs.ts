/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { BatchRunRecordWhereUniqueInput } from "./BatchRunRecordWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { BatchRunRecordUpdateInput } from "./BatchRunRecordUpdateInput";

@ArgsType()
class UpdateBatchRunRecordArgs {
  @ApiProperty({
    required: true,
    type: () => BatchRunRecordWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => BatchRunRecordWhereUniqueInput)
  @Field(() => BatchRunRecordWhereUniqueInput, { nullable: false })
  where!: BatchRunRecordWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => BatchRunRecordUpdateInput,
  })
  @ValidateNested()
  @Type(() => BatchRunRecordUpdateInput)
  @Field(() => BatchRunRecordUpdateInput, { nullable: false })
  data!: BatchRunRecordUpdateInput;
}

export { UpdateBatchRunRecordArgs as UpdateBatchRunRecordArgs };
